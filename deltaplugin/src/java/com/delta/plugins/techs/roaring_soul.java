...
public class roaring_soul implements Listener {
    ...
    static Technique geno = new Technique("geno", "Dreaded Darkness", false, cooldownHelper.minutesToMiliseconds(3), (player, item, args) ->{
    ...
    static Technique darkWorld = new Technique("darkworld", "Dark World", false, cooldownHelper.minutesToMiliseconds(10), (player, item, args) -> {
    ...
    static Technique fallingStars = new Technique(
            ...
            "Falling Stars",
            false,
            cooldownHelper.minutesToMiliseconds(3),
            (player, item, args) -> {
    ...
    static Technique explosionBox = new Technique("blowupbox", "Magic Trap", true, cooldownHelper.secondsToMiliseconds(180), (player, item, args) -> {
    ...
    static Technique cooldownFucker = new Technique("cooldownfunny", "Technique Fucker", false, cooldownHelper.minutesToMiliseconds(5), ((player, itemStack, objects) -> {
    ...
    static Technique roaringgowhacka = new Technique("roaringgowhacka", "Day of the Roaring of Darkness", true, cooldownHelper.hour * 3, (player, item, args) -> {
        ...
        if (phaseNow == 1) {
            ...
            Bukkit.getScheduler().runTaskLater(plugin, () -> {
                ...
                for (Location exLoc : explosionLocs) {
                    ...
                    // Start a repeating task for permanent black dust cascade
                    new BukkitRunnable() {
                        @Override
                        public void run() {
                            // Stop if player is no longer in phase map
                            if (!phase.containsKey(uuid)) {
                                this.cancel();
                                return;
                            }
                            for (int h = 0; h < 5; h++) {
                                Location particleLoc = exLoc.clone().add(0, h, 0);
                                exworld.spawnParticle(Particle.DUST, particleLoc, 20, 0.2, 0.5, 0.2, 0.01, new Particle.DustOptions(Color.BLACK, 2.0f));
                            }
                        }
                    }.runTaskTimer(plugin, 0L, 10L); // 10 ticks = 0.5s, adjust as needed
                }
                new BukkitRunnable(){
                    @Override
                    public void run() {
            ...
            Bukkit.getScheduler().runTaskLater(plugin, () -> {
                ...
                for (Player p : world.getPlayers()) {
                    if (!p.equals(player) && p.getLocation().distance(phase1Center) <= 20) {
                        affected.add(p);
                    }
                }
                ...
                for (String id : com.rschao.plugins.fightingpp.Plugin.getAllFruitIDs()) {
                    for (Technique t : TechRegistry.getAllTechniques(id)) {
                        if (t.getName().equalsIgnoreCase("Fly me to the moon")) continue;
                        int cooldownFinal = Math.max(cooldown, (int) CooldownManager.getRemaining(player, t.getId()));
                        for (Player pl : players) {
