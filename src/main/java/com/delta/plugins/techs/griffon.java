...
public class griffon {
    ...
    static Technique Slizing_Ambush = new Technique(
        "slizing_ambush",
        "Slizing Ambush",
        new TechniqueMeta(false, cooldownHelper.minutesToMiliseconds(1), List.of("Damage and fling nearby entities.")),
    ...
    static Technique Razor_Tail_Flurry = new Technique(
        "razor_tail_flurry",
        "Razor Tail Flurry",
        new TechniqueMeta(false, cooldownHelper.minutesToMiliseconds(2), List.of("Temporarily boost attack speed.")),
        TargetSelectors.self(),
        (ctx, token) -> {
            Player player = ctx.caster();
    ...
    static Technique Feather_Tornado = new Technique(
        "feather_tornado",
        "Feather Tornado",
        new TechniqueMeta(false, cooldownHelper.minutesToMiliseconds(5), List.of("Gather and spiral nearby players while damaging them.")),
        TargetSelectors.self(),
        (ctx, token) -> {
            Player user = ctx.caster();
            Location center = user.getLocation().add(0, 3, 0);
            List<Player> targets;
            if (bossEvents.bossActive && !user.hasPermission("gaster.boss")) {
            ...
            hotbarMessage.sendHotbarMessage(user, ChatColor.LIGHT_PURPLE + "You have used the Feather Tornado technique");
        }
    );
}
