...
public class devourer {
    ...
    static Technique chains = new Technique(
        "deltateodio",
        "Nightmare Chains",
        new TechniqueMeta(false, cooldownHelper.minutesToMiliseconds(5), List.of("Teleport target repeatedly and apply wither.")),
    ...
    static Technique laugh = new Technique(
        "lololol",
        "Chained Explosion",
        new TechniqueMeta(false, cooldownHelper.hour, List.of("Debuff nearby players and deal damage later.")),
        TargetSelectors.self(),
        (ctx, token) -> {
            Player player = ctx.caster();
    ...
    static Technique soulstorm = new Technique(
        "void",
        "Worlds' Last Breath",
        new TechniqueMeta(true, cooldownHelper.hour*3, List.of("Ultimate: complex multi-stage world effect.")),
        TargetSelectors.self(),
        (ctx, token) -> {
            Player player = ctx.caster();
