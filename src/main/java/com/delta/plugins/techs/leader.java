...
public class leader {
    ...
    static Technique flyMeToTheMoon = new Technique(
        "soraingskeis",
        "Soaring Skies",
        new TechniqueMeta(false, 10000, List.of("Propel forward or enable pro flight.")),
    ...
    static Technique plumayubia = new Technique(
        "lluvia",
        "Feather Rain",
        new TechniqueMeta(false, cooldownHelper.minutesToMiliseconds(10), List.of("Schedule multiple arrow spreads.")),
        TargetSelectors.self(),
        (ctx, token) -> {
            Player player = ctx.caster();
            Player p = player;
    ...
    static Technique talon = new Technique(
        "talonflame",
        "Reaction Talons",
        new TechniqueMeta(false, cooldownHelper.minutesToMiliseconds(3), List.of("Damage and fling nearby entities.")),
        TargetSelectors.self(),
        (ctx, token) -> {
            Player player = ctx.caster();
            Location location = player.getLocation();
    ...
    static Technique fallingtwopointo = new Technique(
        "deltastopcopyinshit",
        "Revolution Fireworks",
        new TechniqueMeta(false, cooldownHelper.minutesToMiliseconds(10), List.of("Launch falling obsidian shards that explode.")),
        TargetSelectors.self(),
        (ctx, token) -> {
            Player player = ctx.caster();
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    ...
                    double dist = 5 + rand.nextDouble() * (radius - 5);
