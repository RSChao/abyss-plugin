...
public class manticore {
    ...
    static Technique barbs = new Technique(
        "barbas",
        "Thousand Poison Barbs",
        new TechniqueMeta(false, cooldownHelper.minutesToMiliseconds(3), List.of("Launch a volley of wither arrows.")),
    ...
    static Technique toss = new Technique(
        "toss",
        "Seismic Toss",
        new TechniqueMeta(false, cooldownHelper.minutesToMiliseconds(5), List.of("Launch forward and spawn fireballs.")),
        TargetSelectors.self(),
        (ctx, token) -> {
            Player player = ctx.caster();
    ...
    static Technique llubia = new Technique(
        "yubiaputera",
        "Accid rain",
        new TechniqueMeta(false, cooldownHelper.minutesToMiliseconds(4), List.of("Rain arrows from above around a target.")),
        TargetSelectors.self(),
        (ctx, token) -> {
            Player player = ctx.caster();
